---
title: "Consumption API: Register Product Sales at Tables"
sidebarTitle: "Consumption"
description: "POST /api/consumos records product sales against an active table. GET /api/consumos/mesa/{mesaId} returns open consumptions not yet linked to a session."
---
...
<ResponseField name="sesion" type="Long | null">
  `null` while the consumption is open (table not yet closed). Set to the Sesion `id` once the table is closed.
</ResponseField>
...
| Status | Meaning |
| ------ | ------- |
| `404`  | `mesaId` or `productoId` does not exist. |
...
| Status | Meaning |
| ------ | ------- |
| `404`  | No table with that `mesaId` exists. |
